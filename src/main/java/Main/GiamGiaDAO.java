...
public class GiamGiaDAO {
    ...
    public int addGG(GiamGia gg){
        try {
            String sSQL = "insert KHUYEN_MAI(MAKM, TENKM, LOAI, TRANGTHAI, MUCGG, NGAYAD, NGAYHETHAN, DONVI, MANV)"
                    + "values(?,?,?,?,?,?,?,?,?)";
            ...
            sttm.setString(9, ShareData.taikhoandangnhap.getMaNV());
    ...
    public int updateGG(GiamGia gg){
        try {
            String sSQL = "update KHUYEN_MAI set TENKM = ?, LOAI = ?, TRANGTHAI = ?, MUCGG = ?, NGAYAD = ?, NGAYHETHAN = ?, DONVI = ?, MANV = ? where MAKM = ?";
            con = TestDatabase.getDBConnect();
            sttm = con.prepareStatement(sSQL);
            sttm.setString(9, gg.getMaGG());
            ...
            sttm.setString(8, ShareData.taikhoandangnhap.getMaNV());
    ...
    public ArrayList<String> getListMaSP(){
        ...
        try {
            ...
            System.out.println("Error_GG24:"+e.toString());
        }
        finally{
            try {
                rs.close(); sttm.close(); con.close();
